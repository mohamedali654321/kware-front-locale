...
console.log({visible})
  ...
  useEffect(() => {
    axios
      .get(`http://54.220.211.123:1335/products?_locale=${intl.locale}&_type=supportingProduct`)
      .then((res) => {
        if (!isEqual(res.data, supportProducts)) {
          setSupportProducts(res.data);
        }
        console.log(res.data);
      })
      .catch((err) => console.log(err));
  }, [supportProducts, intl.locale]);
  return (
    <Container>
       <MainProductHeader 
      label={intl.formatMessage({id:"products.related.label"})}
      title={intl.formatMessage({id:"products.related.title"})}
      text={intl.formatMessage({id:"products.related.text"})}
